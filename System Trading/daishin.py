...
class Daishin:
    def __init__(self,msg_on):
        ...
        self.objCssStgList = win32com.client.Dispatch("CpSysDib.CssStgList")  # 전략 리스트
        self.objCssStgFind = win32com.client.Dispatch("CpSysDib.CssStgFind")  # 조건 검색

        self.slack = Slack()
        self.today = datetime.now()
        self.channel_list = {'TEST': '#test_woogie'}
    ...
    def login(self):
        ...
        # 모의투자 계좌 이용시
        # 실행 시 모의투자 부분이 on이 되어있지 않음
        creon.start(f'C:/DAISHIN/STARTER/ncStarter.exe /prj:cp '
                    f'/id:{user_id} /pwd:{user_pwd} /pwdcert:{user_pwdcert} /autostart')
...
"""
    ...
    def send_order(self, bs_type, code, volume):
        # 주식 매수 주문
    ...
    def find_strategies(self, sel):
        retStgList = {}
        # 예제 전략에서 전략 리스트를 가져옵니다.
        if (sel == '예제'):
            self.objCssStgList.SetInputValue(0, ord('0'))  # '0' : 예제전략, '1': 나의전략
        else:
            self.objCssStgList.SetInputValue(0, ord('1'))  # '0' : 예제전략, '1': 나의전략
        self.objCssStgList.BlockRequest()

        # 통신 및 통신 에러 처리
        rqStatus = self.objCssStgList.GetDibStatus()
        if rqStatus != 0:
            rqRet = self.objCssStgList.GetDibMsg1()
            print("통신상태", rqStatus, rqRet)
            return (False, retStgList)

        cnt = self.objCssStgList.GetHeaderValue(0)  # 0 - (long) 전략 목록 수
        flag = self.objCssStgList.GetHeaderValue(1)  # 1 - (char) 요청구분
        print('종목검색 전략수:', cnt)

        for i in range(cnt):
            item = {}
            item['전략명'] = self.objCssStgList.GetDataValue(0, i)
            item['ID'] = self.objCssStgList.GetDataValue(1, i)
            item['전략등록일시'] = self.objCssStgList.GetDataValue(2, i)
            item['작성자필명'] = self.objCssStgList.GetDataValue(3, i)
            item['평균종목수'] = self.objCssStgList.GetDataValue(4, i)
            item['평균승률'] = self.objCssStgList.GetDataValue(5, i)
            item['평균수익'] = self.objCssStgList.GetDataValue(6, i)
            retStgList[item['전략명']] = item
            print(item)

        return (True, retStgList)

    def find_stocks(self, stgName):
        retStgList = {}
        ret, retStgList = self.find_strategies('나의전략')

        id = retStgList[stgName]['ID']

        self.objCssStgFind.SetInputValue(0, id)  # 전략 id 요청
        self.objCssStgFind.BlockRequest()
        # 통신 및 통신 에러 처리
        rqStatus = self.objCssStgFind.GetDibStatus()
        if rqStatus != 0:
            rqRet = self.objCssStgFind.GetDibMsg1()
            print("통신상태", rqStatus, rqRet)
            return (False, retStgList)

        cnt = self.objCssStgFind.GetHeaderValue(0)  # 0 - (long) 검색된 결과 종목 수
        totcnt = self.objCssStgFind.GetHeaderValue(1)  # 1 - (long) 총 검색 종목 수
        stime = self.objCssStgFind.GetHeaderValue(2)  # 2 - (string) 검색시간
        print('검색된 종목수:', cnt, '전체종목수:', totcnt, '검색시간:', stime)

        itemlist = []
        for i in range(cnt):
            item = {}
            item['code'] = self.objCssStgFind.GetDataValue(0, i)
            item['종목명'] = self.objCpStockCode.CodeToName(item['code'])
            itemlist.append(item)

        retStgList[stgName] = itemlist
        return retStgList
